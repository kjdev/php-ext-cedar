...
A typical migration path is to introduce a thin interface in your
application code that both `Aws\VerifiedPermissions\VerifiedPermissionsClient`
and `Cedar\AuthorizationClient` can satisfy:
...
then swap the concrete implementation via dependency injection.
